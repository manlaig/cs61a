...
def balanced(m):
    ...
    def helper(t):
        if is_weight(end(t)):
            return length(t) * size(end(t))
        return length(t) * total_weight(end(t))

    if is_mobile(m):
        return balanced(right(m)) and balanced(left(m)) and helper(right(m)) == helper(left(m))
    elif is_weight(m):
        return True
...
def replace_leaf(t, old, new):
    ...
    return tree(new if label(t) == old and is_leaf(t) else label(t), lst)
...
def make_withdraw(balance, password):
    ...
    def helper(amount, test_pass):
        nonlocal locked, wrong_pass_count, balance, attempts
...
def make_joint(withdraw, old_password, new_password):
    ...
    def helper(amount, test_pass):
        if test_pass == old_password or test_pass == new_password:
            return withdraw(amount, old_password)
        return withdraw(amount, test_pass)
