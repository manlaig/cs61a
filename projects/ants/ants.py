...
class Ant(Insect):
    ...
    # helper function that's called in multiple problems
    def damage_bees(self, damage):
        new_bees = []
        
        for bee in self.place.bees:
            if bee.armor - damage > 0:
                new_bees += [bee]
            else:
                bee.death_callback()
        for bee in new_bees:
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        while begin < self.max_range and (len(curr.bees) == 0 or begin < self.min_range) and curr.entrance != beehive:
            curr = curr.entrance
            begin += 1
...
class ShortThrower(ThrowerAnt):
    ...
    #ThrowerAnt.nearest_bee(self, beehive,0, 5)
    min_range = 0
    max_range = 3
    # END Problem 4
...
class LongThrower(ThrowerAnt):
    ...
    # BEGIN Problem 4
    implemented = True   # Change to True to view in the GUI
    #ThrowerAnt.nearest_bee(self, beehive, float('inf'), 5)
    min_range = 5
    max_range = float('inf')
    # END Problem 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        """
        # BEGIN Problem 5
        dmg = self.damage + amount if self.armor - amount <= 0 else amount
        self.damage_bees(dmg)
...
class HungryAnt(Ant):
    ...
    def action(self, colony):
        ...
        if self.digesting == 0 and self.place.bees != []:
            bee = self.place.bees[random.randint(0,len(self.place.bees) - 1)]
            self.eat_bee(bee)
...
# END Problem 13
    ...
    def action(self, colony):
        ...
        while curr:
            if curr.ant:
                if isinstance(curr.ant, BodyguardAnt) and curr.ant.contained_ant and not curr.ant.contained_ant.is_damage_doubled:
                    curr.ant.contained_ant.damage *= 2
                    curr.ant.contained_ant.is_damage_doubled = True
                if not curr.ant.is_damage_doubled:
                    curr.ant.damage *= 2
                    curr.ant.is_damage_doubled = True
            curr = curr.exit
        # END Problem 13
    ...
    def reduce_armor(self, amount):
        ...
        if self.armor <= 0 and self == QueenAnt.first:
            first = None
            bees_win()
        # END Problem 13
