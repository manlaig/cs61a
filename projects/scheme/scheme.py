...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        # for the extra credit problem 20
        if isinstance(operator, MacroProcedure):
            value = operator.apply_macro(rest, env)
...
def do_and_form(expressions, env):
    ...
    if expressions == nil:
        return True
    answer = scheme_eval(expressions.first, env, expressions.rest == nil)
    return answer if expressions.rest == nil or scheme_falsep(answer) else do_and_form(expressions.rest, env)
    # END PROBLEM 12
...
def do_or_form(expressions, env):
    ...
    if expressions == nil:
        return False
    answer = scheme_eval(expressions.first, env, expressions.rest == nil)
    return answer if scheme_truep(answer) else do_or_form(expressions.rest, env)
    # END PROBLEM 12
