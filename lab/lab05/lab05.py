...
def acorn_finder(t):
    ...
    for node in branches(t):
        found = found or acorn_finder(node)
    return found
...
def sprout_leaves(t, vals):
    ...
    """
    def helper(curr):
        if len(branches(curr)) == 0:
            newbranches = [tree(i) for i in vals]
        else:
            newbranches = [helper(b) for b in branches(curr)]
        return tree(label(curr), newbranches)
